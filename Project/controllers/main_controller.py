...
def cancel_refund():
    ...
    for booking in bookings:
        # indexes unchanged because we APPENDED status in SELECT
        departure_date = (datetime.strptime(booking[5], '%Y-%m-%d').date()
                          if isinstance(booking[5], str) else booking[5])

        db_status = booking[11] if len(booking) > 11 else None  # <-- new
        # Respect cancelled/refunded from DB; otherwise compute for display
        if db_status in ("cancelled", "refunded"):
            status = db_status
        else:
            status = "upcoming" if departure_date > date.today() else "completed"
...
@main_routes.route("/w2")
def w2():
    return render_template("w2.html")

@main_routes.route("/w3")
def w3():
    return render_template("w3.html")

@main_routes.route("/w4")
def w4():
